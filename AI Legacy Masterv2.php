 ...
 * - No duplicate Joomla bootstraps.
 * - settings2.json = training params ONLY
 * - lottery_skip_config.json = lottery specs + DB table + schema
 */
...
// -------------------------------------------------
// Joomla handles + SEO tags (canonical + hreflang)
// -------------------------------------------------
...
// -------------------------------------------------
// Game ID (single source of truth)
// -------------------------------------------------
$input = $app->input;
$rawId = (string) $input->getString('game_id', '');
$rawId = trim($rawId);

$gameId = preg_match('/^[A-Za-z0-9]+$/', $rawId) ? $rawId : '101';

// -------------------------------------------------
// Membership groups (entitlements)
// -------------------------------------------------
$currUser   = $user;
$userGroups = $currUser->groups ?? [];

$defaultGroups = [14]; // fallback paid group
...
$allowedGroups = $gameGroups[$gameId] ?? $defaultGroups;

// Guest check: only allow if group 1 is in allowed groups
if ($currUser->guest && in_array(1, $allowedGroups, true)) {
...
$enthusiastGroupId = 10; // free "Lottery Enthusiast"
$powerballGameId   = '101';

$isEnthusiast = in_array($enthusiastGroupId, $userGroups, true);

// Enthusiast can save only on Powerball. Paid can save anywhere allowed.
$userCanSave = $userIsInAllowedGroup && (!$isEnthusiast || (string) $gameId === (string) $powerballGameId);

// -------------------------------------------------
// Config files (do NOT confuse them)
// -------------------------------------------------
$trainingFile = '/home/oscara/web/lottoexpert.net/public_html/settings2.json';            // AI params only
$specFile     = '/home/oscara/web/lottoexpert.net/public_html/lottery_skip_config.json'; // specs + dbCol + schema

if (!is_file($trainingFile)) {
    die('Training settings file not found.');
}
if (!is_file($specFile)) {
    die('Lottery specs file not found.');
}

$trainingContent = file_get_contents($trainingFile);
$trainingData = json_decode((string) $trainingContent, true);
if (!is_array($trainingData)) {
    die('Training settings JSON is missing or invalid.');
}

$specContent = file_get_contents($specFile);
$specData = json_decode((string) $specContent, true);
if (!is_array($specData) || !isset($specData['lotteries']) || !is_array($specData['lotteries'])) {
    die('Lottery specs JSON is missing or invalid.');
}

// Ensure training settings exist for this game
if (!isset($trainingData[(string) $gameId]) || !is_array($trainingData[(string) $gameId])) {
    die('Game ID not found in training settings.');
}
$gameConfig = $trainingData[(string) $gameId];

// Pull the lottery spec (keyed by gameId) from lottery_skip_config.json
$spec = $specData['lotteries'][(string) $gameId] ?? null;
if (!is_array($spec)) {
    die('Lottery specs missing for game_id=' . htmlspecialchars((string) $gameId, ENT_QUOTES, 'UTF-8'));
}

// Table name for draw lookups (must start with #__)
$tableName = (string) ($spec['dbCol'] ?? '');
$tableName = trim($tableName);

if ($tableName === '' || strpos($tableName, '#__') !== 0) {
    die('Invalid table name pattern in lottery specs: must start with #__');
}

// Schema fields from lottery_skip_config.json
$lotteryConfig = $spec['lotteryConfig'] ?? [];
if (!is_array($lotteryConfig)) {
    die('Invalid lotteryConfig in lottery specs.');
}

$pickSize      = (int) ($lotteryConfig['num_main_balls_drawn'] ?? 0);
$mainNumbersMax= (int) ($lotteryConfig['max_main_ball_number'] ?? 0);

$hasExtraBall  = !empty($lotteryConfig['has_extra_ball']);
$numExtraBalls = (int) ($lotteryConfig['num_extra_balls_drawn'] ?? 0);
$extraBallMax  = (int) ($lotteryConfig['max_extra_ball_number'] ?? 0);

if ($pickSize <= 0 || $mainNumbersMax <= 0) {
    die('Invalid draw schema in lottery specs.');
}
if (!$hasExtraBall) {
    $numExtraBalls = 0;
    $extraBallMax  = 0;
...
// Training params
$epochs             = (int) ($gameConfig['epochs'] ?? 0);
$batchSize          = (int) ($gameConfig['batchSize'] ?? 0);
$dropoutRate        = (float) ($gameConfig['dropoutRate'] ?? -1);
$learningRate       = (float) ($gameConfig['learningRate'] ?? 0);
$activationFunction = (string) ($gameConfig['activationFunction'] ?? '');
$hiddenLayers       = (int) ($gameConfig['hiddenLayers'] ?? 0);
$recencyDecay       = (float) ($gameConfig['recencyDecay'] ?? 0);

// Basic validation (keep strict)
if ($epochs <= 0) {
    die('Invalid value for epochs.');
}
if ($batchSize <= 0) {
    die('Invalid value for batchSize.');
}
if ($dropoutRate < 0 || $dropoutRate > 1) {
    die('Invalid value for dropoutRate.');
}
if ($learningRate <= 0) {
    die('Invalid value for learningRate.');
}
if (!in_array($activationFunction, ['relu', 'sigmoid', 'tanh'], true)) {
    die('Invalid value for activationFunction.');
}
if ($hiddenLayers <= 0) {
    die('Invalid value for hiddenLayers.');
}
...
// Gate endpoint: current URL + ?le_gate=1 (POST + CSRF token required)
    ...
    if ($isEnthusiast) {
        ...
        if ($todayRuns >= $enthusiastDailyCap) {
            echo json_encode([
                'ok' => false,
                'code' => 'ENTH_CAP',
                'message' => 'You have used your 1 Powerball run for today. Come back tomorrow, or upgrade for unlimited access.'
            ]);
            exit;
        }
...
{
    $raw = is_string($tableName) ? trim($tableName) : '';
    if ($raw === '') {
        throw new \RuntimeException('Invalid table name in getNextDrawDate: empty value');
    }

    $suffix = (strpos($raw, '#__') === 0) ? substr($raw, 3) : $raw;
    $suffix = trim((string) $suffix);

    if ($suffix === '') {
        throw new \RuntimeException('Invalid table name in getNextDrawDate: empty suffix');
    }

    if (!preg_match('/^[A-Za-z0-9_]+$/', $suffix)) {
        throw new \RuntimeException('Invalid table name in getNextDrawDate: bad characters');
    }

    $fullTableName = (string) $db->getPrefix() . $suffix;
    ...
    if (!empty($val) && preg_match('/^\d{4}-\d{2}-\d{2}$/', (string) $val)) {
        $val .= ' 00:00:00';
    }
...
// -------------------------------------------------
// Column names & slicing (ASCII-only)
// -------------------------------------------------
$skaiGameType = 'jackpot';
...
// Normalize table name from specs:
// Accept either "#__lotterydb_xx" OR "lotterydb_xx" (suffix only), then build the physical table safely.
// Table name source can vary by config version.
...
if ($rawSpecTable === '') {
    // Keep message simple and safe for production (no dumping full spec)
    die('Invalid table name in lottery specs: missing tableName/dbCol');
}

// If it starts with "#__", strip it; otherwise treat as suffix already.
$tableSuffix = (strpos($rawSpecTable, '#__') === 0) ? substr($rawSpecTable, 3) : $rawSpecTable;
$tableSuffix = trim($tableSuffix);

if ($tableSuffix === '') {
    die('Invalid table name in lottery specs: empty suffix');
}

// Strict allowlist for table suffix: letters, digits, underscore only (prevents injection)
if (!preg_match('/^[A-Za-z0-9_]+$/', $tableSuffix)) {
    die('Invalid table name in lottery specs: bad characters');
}

$fullDbTableName = (string) $db->getPrefix() . $tableSuffix;
...
[[/label]]
    ...
    <!-- PATCH 7A: Live SKAI blend readout (AI ? vs Skip 1-?, plus ? and T) -->
    [[div id="skaiBlendReadout"
      ...
      Mode: balanced ? AI ?: 0.65 ? Skip: 0.35 ? ?: 2.00 ? T: 1.00
    [[/div]]
...
function renderBacktestResults(results) {
  ...
  return tableHTML;
      ...
      // PATCH 7B-1: Live SKAI blend readout (AI ? vs Skip 1-?, plus ? and T)
...
// -- tensors --
		...
		const fitArgs = {
          ...
          // Live blend readout (AI ? vs Skip 1-?, plus ? and T)
